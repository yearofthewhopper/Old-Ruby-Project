 ...
 */
 $(window).on("load resize",function(e){
	...
	*/
	if($('.img-hover').length){
		$(".img-hover.dog-hover-2 .mask-over").each(function(index, val) {
 ...
 */
 (function($) {
	...
	*/
	...
	if($('.dog-data-owl').length){

		$( '.dog-data-owl' ).each(function( index ) {
			var $this =$(this);
			$this.owlCarousel(
			{
				items:$this.data('dog_items'),
				navigation:$this.data('dog_navigation'),
				singleItem:$this.data('dog_singleitem'),
				autoPlay:$this.data('dog_autoplay'),
				itemsScaleUp:$this.data('dog_itemsscaleup'),
				navigationText:['<i class="icon-owl-navigation-left"></i>','<i class="icon-owl-navigation-right"></i>'], 
				pagination:$this.data('dog_pagination'),
				paginationNumbers:$this.data('dog_paginationnumbers'),
				autoHeight:$this.data('dog_autoheight'),
				mouseDrag:$this.data('dog_mousedrag'),
				transitionStyle:$this.data('dog_transitionstyle'),
				responsive:true,
				lazyLoad : true,
				addClassActive:true,
				itemsDesktop :[1450,4],
				afterInit: function(){
					...
					$('.active .caption-content-position', $this).children().each(function(index, val) {
						$(this).addClass('animated '+$(this).data('dog_animation'));
					});
				},
				beforeMove:  function(){
					$('.caption-content-position').children().each(function(index, val) {
						$(this).removeClass('animated '+$(this).data('dog_animation'));
					});
				},
				afterMove:  function(){
					$('.active .caption-content-position', $this).children().each(function(index, val) {
	...
	*/
	if($('.activate-appear-animation').length){
		...
		$('.reloadAnim').click(function (e) {
			$(this).parent().parent().find('img[data-doganim]').attr('class', '').addClass('img-responsive');
			dogAnimAppear();
			e.preventDefault();
		});
	}
 	...
 	if(!$('.swiper-preloader').hasClass('end-loading') && slideNumber != prevSlideNumber){
 		...
 		setTimeout(function() {
 			...
 			.done(function(html) {
 				...
 				$('.dog-Ajax-loader').append('<article class="section-fw animated fadeInUp">'+html+'</article>');
 				$('.swiper-preloader').removeClass('visible');
 				slideNumber++;
 			})
 			.fail(function() {
 			...
 			if($this.parent().parent().is(jQuery(':gt(1)', $mainMenu))){
 				...
 				if($subMenu.css('display') == 'none'){
 					...
 					if($('.header-6').length || $('.header-7').length){
 						var $displayState = ($subMenu.hasClass('dog-mega-menu'))?'table':'block';
 ...
 * @return {[type]} [description]
 */
 function dogAnimAppear(){
 	$("[data-doganim]").each(function() {
 		...
 		if($(window).width() > 1024 && Modernizr.csstransitions) {
 			...
 			$this.appear(function() {
 				...
 				var delay = ($this.data("dogdelay") ? $this.data("dogdelay") : 1);
 				if(delay > 1) $this.css("animation-delay", delay + "ms");

 				$this.addClass("animated");
 				$this.addClass($this.data("doganim"));
 ...
 */
 function toTop(mobile){
 	...
 	if(mobile == false){
 		...
 		if(!$('#dogToTop').length)
 			$('body').append('<a href="#" id="dog-to-top"><i class="icon-to-top"></i></a>');

 		$(window).scroll(function () {
 			if ($(this).scrollTop() > 100) {
 				$('#dog-to-top').slideDown('fast');
 			} else {
 				$('#dog-to-top').slideUp('fast');
 			}
 		});

 		$('#dog-to-top').click(function (e) {
 			e.preventDefault();
 			$("html, body").animate({
 				scrollTop: 0
 			}, 800, 'easeInOutCirc');
